...
module.directive( 'gravatar', () => {
  ...
  function buildGravatarUrl( email, size, type ) {
    ...
    var hash = window.md5( email.trim().toLowerCase() );
  ...
  return {
    ...
    link: function( scope : any ) {
      scope.$watchGroup( ['email', 'size'], () => scope.gravatarUrl = buildGravatarUrl( scope.email, scope.size, scope.type ) );
    }
  };
} );